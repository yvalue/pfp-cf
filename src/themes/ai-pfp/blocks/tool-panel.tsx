...
export function ToolPanel({
  ...
  const commonT = useTranslations('common.tool_panel');

  // Merge section config with common defaults
  const tabs = section.tabs || commonT.raw('tabs');
  const fields = section.fields || commonT.raw('fields');
  const upload = section.upload || commonT.raw('upload');
  const description_placeholder = section.description_placeholder || commonT('description_placeholder');
  const selects = section.selects || commonT.raw('selects');
  const result = section.result || commonT.raw('result');
  const buttons = section.buttons || commonT.raw('buttons');
  const credits = section.credits || commonT.raw('credits');
  ...
  const costPerImage =
    resolvedGeneration?.costCredits ?? credits.submit_cost ?? 0;
  ...
  const countOptions = useMemo(
    () =>
      selects.batch_size.filter((item) => {
    ...
    [selects.batch_size, selectedModelFamily]
  );
  ...
  const uploadSupportsText = useMemo(() => {
    const template = upload.supports?.trim();
  ...
  }, [remainingUploadSlots, upload.supports]);
            ...
            <TabsContent value="upload" className="mt-0 flex-1">
              <div className={sectionClassName}>
                <section className="grid gap-2">
                  <FieldHeader
                    badge={fields.required_badge}
                    badgeVariant="required"
                    label={fields.upload_label}
                  />
                ...
                <section className="grid gap-2">
                  <FieldHeader
                    badge={fields.optional_badge}
                    label={fields.description_label}
                    trailing={
                ...
                <section className="grid gap-4">
                  <FieldHeader
                    badge={fields.optional_badge}
                    label={fields.effect_style_label}
                  />
            ...
            <TabsContent value="parameter" className="mt-0 flex-1">
              <div className={sectionClassName}>
                <div className="grid min-w-0 gap-2">
                  <FieldHeader
                    badge={fields.default_badge}
                    label={fields.model_label}
                  />
                ...
                <div className="grid gap-4">
                  <div className="grid min-w-0 gap-2">
                    <FieldHeader
                      badge={fields.default_badge}
                      label={fields.aspect_ratio_label}
                    />
                  ...
                  <div className="grid min-w-0 gap-2">
                    <FieldHeader
                      badge={fields.default_badge}
                      label={fields.count_label}
                    />
                    <Select value={countValue} onValueChange={setCountValue}>
                      <SelectTrigger className={selectTriggerClassName}>
                        <SelectValue placeholder={fields.count_label} />
                      </SelectTrigger>
                      <SelectContent>
              ...
              <section className={`${panelClassName} bg-background p-6`}>
                <header>
                  <h3 className="text-foreground text-center text-lg font-semibold md:text-xl">
                    {result.how_to_use_title}
                  </h3>
                </header>
                <div className="mt-4 grid gap-3">
                  {result.usage_steps.map((step, index) => (
